...
const cartController = {
    ...
    addToCart: async (req, res, next) => {
        try {
            ...
            if (isNaN(q)) { return next(new AppError('quantity must be a number', 400)); }

            if (q <= 0) { return next(new AppError('quantity must be equal 1 or more', 400)); }
    ...
    updateItemQuantity: async (req, res, next) => {
        try {
            ...
            if (isNaN(q) || q <= 0) { return next(new AppError('quantity must be a whole number 1 or more', 400)); }
